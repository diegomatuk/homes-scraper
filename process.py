...
# df['longitude'] = lon
# df['latitude'] = lat
df = pd.read_csv('outputs/output_houses.csv')
#PREDICTIONS


df['pred_alquiler'] = df.apply(df_class.pred_alquiler,axis = 1)
df['alquiler_anual'] = df.pred_alquiler * 12
df['imp_alcabala'] = df.apply(df_class.imp_alcabala,axis = 1)
df['rentabilidad'] = df.apply(lambda x: x['alquiler_anual']/(x['Venta'] + x['imp_alcabala']),axis = 1)


df.to_csv('outputs/output_houses.csv')
